...
CREATE VIEW IF NOT EXISTS resumen_compra AS SELECT COUNT( id ) as cantidad, fecha, strftime( fecha, '%m' ) as mes, date(fecha, '%Y' ) as ano, SUM( total ) total, id_proveedor, contado FROM compras;