...
CREATE VIEW IF NOT EXISTS resumen_compra AS SELECT COUNT( id ) as cantidad, MAX( fecha ) as fecha, strftime("%m" , max( fecha ) ) as mes, strftime( "%Y", MAX( fecha ) ) as ano, SUM( total ) as total, id_proveedor, contado FROM compras