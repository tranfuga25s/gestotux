...
bool BajaClienteServicioTest::hacerSql( QString nombre )
{
 if( QFile::exists( ":/sql/"+nombre+"."+QSqlDatabase::database( QSqlDatabase::defaultConnection, false ).driverName()+".sql" ) )
 {
        ...
        {
                qWarning(qPrintable( "Error al abrir el archivo: :/sql/"+nombre+"."+QSqlDatabase::database().driverName()+".sql" ) );
                return false;
        }
 }
 else
 {
  qWarning( qPrintable( "No se pudo generar las tablas del test " + nombre + ". No se encontro el archivo: :/sql/"+nombre+"."+QSqlDatabase::database( QSqlDatabase::defaultConnection, false ).driverName()+".sql" ) );
  return false;
 }
}
